...
function connectToAPI() {
  ...
  server_ws.onclose = function (msg) {
    if (msg.code !== 1001) {
      ...
      console.warn(
        `API WebSocket CLOSED: ${msg.code} ${msg.reason}, trying again`,
      );
      connectToAPI();
    }
  };
...
function connectController() {
  $.get({
    ...
    controller_ws = new WebSocket(`ws://localhost:${data.port}`);
    ...
    controller_ws.onmessage = function (message) {
      ...
      switch (data.type) {
        case "ServerHandshake":
          ...
          break;
        case "BotListUpdate":
          console.log("BotListUpdate", data.bots);
          ...
          if (connected_bots.length == 0) {
            ...
            console.debug("Connected bots: ", connected_bots);
    ...
    controller_ws.onclose = function (msg) {
      if (msg.code !== 1001) {
        // server is 'going away'
        console.warn(
          `CONTROLLER WebSocket CLOSED: ${msg.code} ${msg.reason}, trying again`,
        );
...
function createBotUpdates(gameStartData) {
  gameStartData.participants.forEach((bot) => {
    if (!$(`#botUpdate_${bot.id}`).length) {
      $("#botUpdates").append(
        `<tr id="botUpdate_${bot.id}" class="botUpdate"><td class="botUpdate_id">${bot.id}</td><td class="botUpdate_name">${bot.name}</td><td class="botUpdate_energy">?</td><td class="bd">0</td><td class="bkb">0</td><td class="lsb">0</td><td class="rd">0</td><td class="rkb">0</td><td class="surv">0</td><td class="TS">0</td></tr>`,
      );
    }
  });
}
...
function updateBotUpdates(tickData) {
  tickData.botStates.forEach((botState) => {
    ...
    $(".energy", $(`#${PID}_label`)).text(
      `E:${Math.round(botState.energy, 2)}`,
    );
    $(".botUpdate_energy", `#botUpdate_${botState.id}`).text(
      Math.round(botState.energy, 2),
    );
  });
}
...
function addRunningBotTab(pid, name) {
  ...
  $("#stdout_tabs").append(
    `<input class="tab_input running_bot ${item_class}" id="${pid}" type="radio" name="stdout_tab">`,
  );
  $("#stdout_tabs").append(
    `<label id="${pid}_label" class="tab_label running_bot ${item_class} has-text-centered" for="${pid}" title="NAME:${name} PID:${pid}">${name}<br><p class="energy">E:?</p></label>`,
  );
  $("#stdout_tabs").append(
    `<div class="tab_content running_bot ${item_class}"><textarea class="textarea is-small has-fixed-size" id="${pid}_content" rows="21" name="${pid}_content" placeholder="Console output for ${name}(${pid})" readonly="readonly" autocomplete="off"></textarea></div>`,
  );
  if ($(".tab_label").length == numberOfParticipants + 1) {
    createToolTips();
  } else {
    console.info(
      "tootltips nto created",
      $(".tab_label").length,
      numberOfParticipants,
    );
  }
...
function checkConnections() {
  if (last_serverWS_state !== server_ws.readyState) {
    //changed!
  ...
  if (last_controllerWS_state !== controller_ws.readyState) {
    //changed!
...
$(document).ready(function () {
  ...
  connectionCheck_interval = setInterval(
    checkConnections,
    connectionCheck_speed,
  );
