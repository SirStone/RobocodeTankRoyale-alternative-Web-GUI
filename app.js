...
bot_roots.push({ tag:`Sample Bots V${version}`, path: `${sample_bots_dir}/sample-bots-java-${version}`})
...
app.use('/jquery-ui', express.static(path.join(__dirname, '/node_modules/jquery-ui'))) //setting '/jquery-ui' pointing to the jquery-ui npm module folder
app.use('/fa', express.static(path.join(__dirname, '/node_modules/@fortawesome/fontawesome-free'))) // setting '/fa' pinting to the font-awesome npm module folder
...
function runServer() {
  ...
  serverProcess = spawn("java -XX:+SuppressFatalErrorMessage", ["-jar", `${process.env.SERVERDIR}/robocode-tankroyale-server-${version}.jar`, `--port=${server_port}`, `--botSecrets=${botsecret}`, "--controllerSecrets=controllersecret"], { shell: true })
